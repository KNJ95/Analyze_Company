...
`マイナビ2027 corp${corpId} の企業情報です。

---
${text.slice(0,4000)}
---

Be-Ready評価軸: ${AXES.map(a=>`${a.id}.${a.name}`).join(" / ")}
...
export default function App() {
  ...
  const [raw,        setRaw]        = useState([]);   // jobs_raw.json の未分析データ
  ...
  useEffect(() => {
    ...
    fetch("/jobs_raw.json")
      ...
      .then(data => {
        setRaw(data);
        addLog(`📄 jobs_raw.json から ${data.length}件 読み込みました`);
      })
      .catch(e => {
        setLoadErr(e.message);
        addLog(`jobs_raw.json が見つかりません — scraper.py を実行してください`, "error");
      });
  }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if(rest.length > 0) await sleep(300); // AI APIには間隔不要
      setProcessing(false);
    })();
  }, [queue, processing]);
              ...
              {loadErr ? (
                <div style={{...sty.card,borderColor:`${C.error}55`}}>
                  ...
                  <p style={{fontSize:13,color:C.sub,lineHeight:1.8}}>
                    まず <code style={{background:C.surface2,padding:"1px 6px",borderRadius:4,color:C.primary}}>scraper.py</code> をローカルで実行してください。<br/>
                    生成された <code style={{background:C.surface2,padding:"1px 6px",borderRadius:4,color:C.primary}}>public/jobs_raw.json</code> をVercelにデプロイするとここに反映されます。
                  </p>
                </div>
              ) : (
                    ...
                    {filtered.map((j,ri)=>(
                      <tr key={j.id}>
                        ...
                        <td style={{...sty.td(ri),maxWidth:150,fontWeight:600}}>
                          <a href={`https://job.mynavi.jp/27/pc/search/corp${j.corpId}/outline.html`} target="_blank" rel="noreferrer"
                        ...
                        {AXES.map(a=>{
                          ...
                          return <td key={a.id} title={`${a.name}: ${sc}`} style={{...sty.td(ri),textAlign:"center",padding:"6px 3px",background:SCORE_BG[sc]||SCORE_BG[0],color:SCORE_FG[sc]||SCORE_FG[0],fontWeight:sc>=4?700:400,fontSize:13}}>{sc||"—"}</td>;
                        })}
                    ...
                    {Object.entries(summary).sort((a,b)=>b[1].count-a[1].count).map(([ind,d],i)=>(
                      <tr key={ind} style={{background:i%2===0?C.surface:C.surface2}}>
                        ...
                        {AXES.map(a=>{
                          ...
                          return <td key={a.id} title={`${a.name}: ${v}`} style={{textAlign:"center",padding:"8px 4px",background:SCORE_BG[sc]||SCORE_BG[0],color:SCORE_FG[sc]||SCORE_FG[0],fontWeight:sc>=4?700:400,fontSize:13,borderBottom:`1px solid ${C.border}`}}>{v}</td>;
                        })}
                      </tr>
                    ))}
                  </tbody>
